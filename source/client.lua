...
if config["/panic"].enabled then
    ...
    AddEventHandler("ND_Panic:panic", function(coords)
        local player = NDCore.getPlayer()  -- Retrieve player data
        local character = player  -- Access character data directly
        ...
        if ignoreDepartments then
            ...
            -- Check if the player's job matches allowed departments
